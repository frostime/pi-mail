...
Use `mail` only when information must cross a session boundary. A Pi footer status such as `mail 2` means this session has unpresented mailbox entries; inspect the inbox when they are relevant. Discover peers when you do not know the recipient. `to` and `cc` may contain several recipients. They accept aliases, full session IDs, or unambiguous ID fragments of at least six characters. The displayed session short ID uses the UUID tail because nearby Pi sessions can share a long leading prefix; the displayed short ID is directly addressable.

Peer mail is quiet by default. Set `notify: true` only when direct `to` recipients should be interrupted immediately; `cc` remains informational. Ordinary pending direct mail may produce a lightweight count-only reminder instead of injecting every message body. A human user may also enable a stale-mail reminder for a mailbox; do not rely on that policy for correctness or treat the reminder as a task assignment. Mail from the reserved `user` principal originates from the human Web UI and is delivered as a genuine Pi user message.
