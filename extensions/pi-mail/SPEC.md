...
The LLM-facing surface remains one `mail` tool. Its registration metadata must stay deliberately small; usage policy and examples belong in the bundled `pi-mail` skill rather than `promptGuidelines` or long parameter descriptions. Tool `content` should carry only the information needed for the next model action instead of mirroring storage JSON. Structured values may remain in `details` for rendering and session state.

## Identity, discovery, and session replacement

A Pi session UUID is the immutable mailbox identity. New sessions receive a compact generated alias in the form `S###`, where the number is derived from the session ID and advanced until it is unused in the project. This generation avoids collisions among automatic aliases; explicitly configured aliases remain mutable user-chosen addresses and are not required to be unique. Pi's conversation/session display name is stored separately from the mailbox alias so the human UI can show both without conflating them. Session short IDs use the random UUID tail rather than the leading timestamp-like portion; nearby time-ordered session UUIDs can otherwise share the same visible prefix. Address resolution accepts an exact session ID, an unambiguous leading or trailing ID fragment of at least six characters, or an alias; ambiguity fails rather than silently selecting a peer. Message references used by `reply_to`, `inbox`, and `thread` likewise accept exact IDs or unambiguous ID fragments.
...
Ordinary peer mail must not inject its body into the recipient Pi context merely because it arrived. When `notify: true` is set on a direct `To` delivery, the recipient Pi process inserts a `pi-mail` custom message with `deliverAs: "steer"` and `triggerTurn: true`. `Cc` remains silent even when the message carries the flag. Silent direct `To` mail is summarized after backlog accumulation: the current adapter threshold is three pending direct messages, and notices occur at new threshold buckets rather than once per poll. `Cc` mail is excluded from wake-up thresholds. Such a notice reports only the pending count and does not mark those messages as presented.

A stale-mail reminder is an optional Pi-adapter attention policy owned by the human user, not a message field and not an orchestration primitive. It is disabled by default and configured per mailbox with `/mail-reminder off|<minutes>`. When enabled, quiet unpresented direct `To` mail may trigger one count-only follow-up after the configured age. It must not trigger for `Cc`, `notify: true`, human-origin, or already-presented mail. Within one continuous backlog episode the adapter should avoid repeated stale reminders; clearing the direct pending backlog rearms the policy. Web UI observation is supervisory only and must not mutate `presentedAt` or suppress the recipient-side stale policy.

The Pi adapter exposes the current mailbox's unpresented count through an extension footer status. This status is informational UI state only; it must not change delivery or presentation state.

Model-facing `inbox` without `message_id` is a list view and must use bounded body previews. `thread` is also a bounded preview view. `inbox` with a specific `message_id` returns that received message in full. `wait` is also preview-oriented rather than a full-message dump. These rules prevent a single long mailbox, wait result, or thread lookup from flooding model context; the exact preview character limit is an implementation detail.
...
`wait` is an inbox-level synchronization primitive, not an orchestration primitive. It does not target a sender, thread, task, or workflow state. Before blocking it snapshots current delivery IDs and checks existing unpresented mail; if pending mail already exists, it returns immediately. Otherwise any delivery absent from that snapshot satisfies the wait. This ordering must preserve the no-lost-wakeup property even when a message arrives around wait startup.

A wait is always finite and abortable. The adapter default is 60 seconds and the public tool caps `timeout_seconds` at 300 seconds. Timeout means only that no mail satisfied the wait in that interval. `wait` is deliberately non-consuming: returning its preview must not advance `presentedAt`. This preserves human/notify delivery behavior and ensures a still-unhandled pending message can satisfy a later wait again until the agent explicitly inspects its inbox.
...
The human supervisor view may list the current session and sessions hidden from peer discovery because `discoverable` controls peer discovery, not local-user visibility. The compose view may select one, several, or all currently active sessions; “all active” expands to an explicit `To` list and does not introduce broadcast/group semantics into mail-core. Session and message lists must remain scrollable rather than allowing project history to grow the page without bound. Session cards must expose pending To/Cc counts and enough age information for the human supervisor to notice stalled mailboxes without opening every session. This supervisor view must not mark those messages presented.
